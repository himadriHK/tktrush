...
require("connection1.php");
	...
	//echo $file_path;
	$query_guidecatRs = "SELECT * FROM `banner2`";
...
$banners= mysql_fetch_array($guidecatRs);
	...
	$status=$_POST['status'];
	$sqlstmt= "UPDATE banner2 SET url='$url',status='$status' WHERE id=1";
	...
	$image = $_FILES['upload']['type'];
	  ...
	  {
	       @unlink($_SERVER['DOCUMENT_ROOT'].$banners['image']);
	       $tmp_path=$_FILES['upload'][tmp_name];
	      
	       
	       if(move_uploaded_file($tmp_path,$file_path.$img_path))
	       {
	         $sqlstmt= "UPDATE banner2 SET image='$img_path' WHERE id=1";
	         $data=mysql_query($sqlstmt)or die(mysql_error());
	         if($data)
	         {
...
<tr>	</tr>
      ...
      <table width="98%" border="1" cellspacing="0" cellpadding="0" bordercolor="#F3F3F3">
        <tr bgcolor="#CCCCCC"> 
          ...
          <td width="10%" height="19"><b><font face="Verdana, Arial, Helvetica, sans-serif" color="#CC3300" size="2">Status</font></b></td>
        ...
        <tr valign="top"> 
          <td width="30%"><font color="#CC3300" face="Verdana, Arial, Helvetica, sans-serif" size="2"> 
            <img src="<?php echo $banners['image'];?>" border='0' height='107' width='540'>
