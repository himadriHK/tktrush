...
if($yes)
{
	...
	$query = "Delete from tblphoto where photoid=$del";
	mysql_query($query);
}
...
if($update)
{
	...
	if($userfile=="none")
		{
			$query_up = "UPDATE tblphoto SET description='$description',image='$image' where photoid=$photoid";
			mysql_query($query_up);	
		}
	else
		{			
			if (@copy ($userfile,"../photoimages/".$userfile_name))
			{
				//echo"Image Copied";
			}
			
						
			$query_up = "UPDATE tblphoto SET description='$description',image='$userfile_name' where photoid=$photoid";
			mysql_query($query_up);
...
<tr>	  
  	<?
$query1= "SELECT * FROM tblphotocategory";
...
$catimage=$myrow[2];
     ...
     echo"<td align=center><a href=$PHP_SELF?more=$catid><font face=verdana size=1 color=#cc3300><img src=../photoimages/$catimage border=0 width=75 height=75></font></a><br>";
...
<form action=<?echo"$PHP_SELF";?> method=post>
        <table width="100%" border="1" cellspacing="0" cellpadding="0" bordercolor="#F3F3F3">
          ...
          <?
$result = mysql_query( "SELECT * FROM tblphoto where catid=$more");
...
?>
          <tr valign="top"> 
            <td width="18%"><font color="#CC3300" face="Verdana, Arial, Helvetica, sans-serif" size="2"> 
              <?echo"<img src=../photoimages/$image border=0 height=75 width=75>";?>
              </font></td>
            ...
            <td width="10%" align="center"><font face="Verdana, Arial, Helvetica, sans-serif" size="1" color="#CC3300"> 
              <?echo"<a href=photoedit.php?photoid=$photoid><font color=#cc3300>Edit</font></a>";?>
              </font></td>
            <td width="11%" align="center"><font face="Verdana, Arial, Helvetica, sans-serif" size="1" color="#CC3300">
              <?echo"<a href=photodel.php?del=$photoid&more=$more><font color=#cc3300>Delete</font></a>";?>
              </font> </td>
          </tr>
          <?
}
