	...
	<div class="main-wrapper">
		<div class="container-fluid">
			...
			function listRecords() {
				...
				$data=$database->query("select e.title as Title,o.ticket_price,count(*)*(o.tickets+o.ctickets) as tickets_sold from 
ticket_orders o left outer join partners p on o.partner_id=p.spid, events e 
where e.tid=o.tid and o.payment_status='paid' group by e.title,o.ticket_price")->fetchAll(PDO::FETCH_ASSOC);
//var_dump($data);
   ...
   // $unitPriceField->type('number');
	...
	$group->field('Title')->addAggregate($ticketsSoldField);
    ...
    $unitPriceColumn->field('ticket_price')
                    ->width('130px')
                    ->format('{0:c}')
                    ->title('Ticket Price');
    
    $ticketsSoldField = new \Kendo\UI\GridColumn();
    $ticketsSoldField->field('tickets_sold')->title('Tickets Sold')->width('130px')->groupFooterTemplate('Total: #=sum#');
...
$pdf->allPages(true)
    ...
    $grid->pdf($pdf)->addToolbarItem(new \Kendo\UI\GridToolbarItem('pdf'))->addColumn($unitPriceColumn, $ticketsSoldField)->dataSource($dataSource)->sortable(true)->groupable(false)->filterable(true)->columnMenu(true)->pageable($pageable);
