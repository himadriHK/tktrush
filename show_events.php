...
if(strpos($pageSelected, 'out_and_about.php'))
{
    if (!$_SESSION['PP_UserId']){
        ...
        $query_eventRs = "SELECT events.*, promoters.name, promoters.phone  FROM events, promoters WHERE events.promoter = promoters.spid  AND ongoing='".OUTANDABOUT."' AND city='{$default_city_id}' AND partner='{$_SESSION['PP_UserId']}' AND date_end>='".date('Y-m-d')."'  $query_string ORDER BY hot ASC, ongoing DESC, tid DESC";
...
else if(strpos($pageSelected, 'ongoing_events.php'))
{
    if (!@$_SESSION['PP_UserId']){
        ...
        $query_eventRs = "SELECT events.*, promoters.name, promoters.phone  FROM events, promoters WHERE events.promoter = promoters.spid  AND ongoing='".ONGOING."' AND city='{$default_city_id}' AND partner='{$_SESSION['PP_UserId']}' AND date_end>='".date('Y-m-d')."'  $query_string ORDER BY hot ASC, ongoing DESC, tid DESC";
//and sale_date <= CURDATE()
    }
...
<link type="text/css" rel="stylesheet" href="css/TM_style.css">
<table width="100%" cellspacing="0" cellpadding="0" border="0">
<tbody><tr>
<td class="dot_line" style="height:22px;" colspan="5">&nbsp;</td>
</tr>
<tr>
